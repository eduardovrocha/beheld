...
| F0.3 | Build do binário TypeScript com Bun `--compile` | ✅ | Gerar `dist/devprofile` standalone |
| F0.4 | Build do binário Python com PyInstaller `--onefile` | ✅ | Gerar `dist/devprofile-engine` standalone |
...
| F0.11 | Smoke test: `devprofile --version` sem Node/Python no PATH | ✅ | Critério de conclusão da fase |
...
| ID | Item | Status | Notas |
|----|------|--------|-------|
| F1.6 | Classe `JsonlWriter` — append e criação de arquivo | ✅ | `~/.devprofile/sessions/YYYY-MM-DD_<uuid>.jsonl` |
...
| ID | Item | Status | Notas |
|----|------|--------|-------|
| F1.35 | Tool `devprofile` — busca scores e formata perfil | ✅ | Parâmetro `view`: summary/scores/insights/full |
| F1.36 | Tool `devprofile_status` — retorna score atual para sidebar | ✅ | Para Continue.dev na Fase 4 |
| F1.37 | Registro das tools no endpoint `/mcp` | ✅ | |

#### Escopo global do /devprofile
...
| F1.43 | `devprofile init` detecta e corrige registro em escopo de projeto | ✅ | Remove de `~/.claude/projects/*/settings.json` se encontrar |
| F1.44 | Registro no Continue.dev em `~/.continue/config.json` — escopo global | ✅ | Não em `.continue/config.json` local do workspace |
| F1.45 | `devprofile init` nunca cria ou edita arquivos de config dentro do CWD | ✅ | Proibido: `./.claude/`, `./.continue/` — apenas `~/` |
| F1.46 | Validação cruzada: `/devprofile` sem prompt em projeto diferente do instalado | ✅ | Teste: init em `~/projects/a`, invocar em `~/projects/b` sem confirmação |
| F1.47 | Wizard tela 3 informa: "disponível em todos os projetos, sem confirmação" | ✅ | |
| F1.48 | `devprofile delete --all` remove de `~/.claude/settings.json` e de qualquer escopo de projeto | ✅ | Limpeza completa |

#### Daemon e autostart

| ID | Item | Status | Notas |
|----|------|--------|-------|
| F1.38 | PID gravado em `~/.devprofile/daemon.pid` | ✅ | |
| F1.39 | LaunchAgent para macOS | ✅ | `com.devprofile.daemon.plist` |
| F1.40 | Systemd user service para Linux | ✅ | `devprofile.service` |
| F1.41 | Log com rotação em 10 MB | ✅ | `~/.devprofile/daemon.log` |
...
| Arquivo JSONL em `~/.devprofile/sessions/` com nome correto | ✅ |
| Hook Stop dispara processamento do engine em background | ✅ |
| Daemon reinicia após reboot | ✅ |
| `/devprofile` responde em projeto diferente sem prompt de confirmação | ✅ |
...
| F2.9 | Classe `JsonlReader` com cursor persistente | ✅ | `~/.devprofile/.cursor` |
...
| F2.52 | Smoke test: `./dist/devprofile-engine` sem Python no PATH | ✅ | |
...
| F3.2 | `engine-extractor.ts` — extrai binário embarcado | ✅ | `~/.devprofile/bin/engine` |
...
| F3.10 | Comando `devprofile init` | ✅ | |
| F3.11 | Comando `devprofile start / stop / status` | ✅ | |
| F3.12 | Comando `devprofile view` com flags `--json`, `--scores-only`, `--since`, `--dimension` | ✅ | `--refresh` força reprocessamento se há eventos órfãos |
| F3.13 | Comando `devprofile update` com verificação de SHA256 | ✅ | |
| F3.14 | Comando `devprofile delete` com confirmação obrigatória | ✅ | |
| F3.15 | Subcomando `devprofile server` para autostart do SO | ✅ | Inicia apenas o MCP server |
| F3.16 | Salva config em `~/.devprofile/config.json` | ✅ | Dimensões ativas, portas, notificações |
...
| ID | Item | Status | Notas |
|----|------|--------|-------|
| F4.1 | Tool `devprofile_status` registrada no MCP server | ✅ | Score, sessions_today, top_insight |
| F4.2 | Tool `devprofile` — views: summary, scores, insight, full | ✅ | Plain text legível no chat do Continue.dev |
| F4.3 | `NotificationService.send()` — macOS e Linux | ✅ | osascript / notify-send |
| F4.4 | `shouldNotifyToday()` — controle por tipo e data | ✅ | `~/.devprofile/notifications.json` |
...
| F4.9 | Teste manual: `/devprofile` no chat do Continue.dev | ✅ | |
...
| ID | Item | Status | Notas |
|----|------|--------|-------|
| F5.1.1 | Geração automática no `devprofile init` se chave não existir | ⬜ | `~/.devprofile/keys/private.pem` com permissão `0600` |
| F5.1.2 | Chave pública exportável em `~/.devprofile/keys/public.pem` | ⬜ | |
| F5.1.3 | `devprofile keys show` — exibe chave pública atual | ⬜ | |
| F5.1.4 | `devprofile keys import <path>` — importa chave Ed25519 existente | ⬜ | Valida formato antes de substituir |
| F5.1.5 | `devprofile keys rotate` — gera novo par, mantém histórico de chaves públicas | ⬜ | Snapshots antigos continuam verificáveis com chave embutida |
...
| ID | Item | Status | Notas |
|----|------|--------|-------|
| F5.3.1 | `devprofile snapshot` — comando CLI principal | ⬜ | |
...
| F5.3.6 | Salva `.dpbundle` em `~/.devprofile/snapshots/` e no path especificado por `--output` | ⬜ | |
| F5.3.7 | `devprofile snapshot list` — lista histórico com data, overall e hash | ⬜ | |
| F5.3.8 | `devprofile verify <arquivo.dpbundle>` — verificação offline | ⬜ | Verifica hash + assinatura + cadeia |
...
| ID | Item | Status | Notas |
|----|------|--------|-------|
| F5.4.1 | `devprofile snapshot --share` — upload para portal + QR | ⬜ | |
| F5.4.2 | QR code no terminal via unicode block chars (sem dependência de imagem) | ⬜ | Lib: `qrcode-terminal` |
| F5.4.3 | URL curta retornada pelo portal: `devprofile.app/v/<id>` | ⬜ | |
...
| F5.5.5 | Badge embed: `<img src="devprofile.app/v/:id/badge.svg">` | ⬜ | Para README e LinkedIn |
...
#### F6.6 — CLI: comando `devprofile import`
...
[1] Importar agora
[2] Importar depois  (devprofile import)
[3] Pular
```
...
**Output do `devprofile snapshot`:**
```
Base histórica:       12 repositórios · 4.832 commits
Trajetória observada: 847 sessões · 90 dias
```

**Critério de conclusão:** bundle gerado contém chaves `"l1"` e `"l2"` como objetos distintos · `devprofile verify` valida L1 e L2 separadamente
...
```
[ ] devprofile import <url> clona, extrai sinais e descarta clone
...
[ ] devprofile import --list exibe tabela correta
[ ] Wizard devprofile init oferece Tela 3.5 de bootstrap
...
| D2 | Fallback quando engine offline no `/devprofile` | ✅ resolvido | `source: "cache"` via `bun:sqlite` direto. View exibe aviso com data. `process.exit(1)` sem nenhum dado. |
...
| D5 | Nome do repositório no GitHub | ⚠️ pendente | Sugestão: `devprofile/devprofile` |
| D6 | Domínio do produto | ⚠️ pendente | `devprofile.app` assumido na spec — confirmar |
...
|----|-----|---------|--------|
| B1 | `devprofile start` e `devprofile init` não detectam daemons já em execução | Médio | ✅ resolvido — `isMcpRunning()` + `isEngineRunning()` via `/health` antes de qualquer spawn |
| B2 | `devprofile server` sobe HTTP server em vez de implementar protocolo MCP stdio | Alto | ✅ resolvido — `StdioServerTransport` do SDK MCP. Flag `--stdio` explícito em `args`. |
...
| B5 | Permissões `~/.devprofile` são `0755` em vez de `0700` | 🔴 Crítico | ✅ resolvido — `mode: 0o700` em todos os `mkdirSync`. `ensureSecurePermissions()` corrige instalações existentes. |
| B6 | Engine não é iniciado pelo LaunchAgent após reboot — só porta 7337 sobe | Alto | ✅ resolvido — LaunchAgent e systemd usam `devprofile start`. |
| B7 | `devprofile view --json` polui stdout com warnings — quebra pipe com `jq` | Alto | ✅ resolvido — warnings redirecionados para `process.stderr` quando `--json` ou `--scores-only`. |
...
| Slash command | ⚠️ ausente nesta máquina de dev — código correto, `devprofile init` cria no primeiro uso |
...
| 2026-05-10 | Flag `--refresh` em `devprofile view` para eventos órfãos | F3.12 |
| 2026-05-10 | `/devprofile` global: registro em `~/.claude/settings.json` evita prompt de confirmação por projeto | F1.42–F1.48, F3.6–F3.7 |
...
| 2026-05-11 | B6 corrigido: LaunchAgent/systemd usam `devprofile start` | 6/6 — 63 testes |
