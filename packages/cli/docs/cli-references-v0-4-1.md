# Beheld — Referência do CLI v0.4.1

> Fonte: `packages/cli/src/` (commit `afa2b6a` · 2026-06-08 · VERSION `0.4.1`)
> Documento gerado por varredura do código-fonte. O código é autoritativo — specs ficam atrás dele quando divergem.
> Constante canônica em `packages/cli/src/version.ts`, re-exportada por `packages/cli/src/index.ts` e importada por `commands/init.ts`, `commands/update.ts` e `ui/wizard.ts`.
...
**Pré-condições:** rede; chave Ed25519 (gerada se faltar). API do portal acessível.
...
**Notas.** (não executado — efeito colateral: escreve attestation no disco e fala com o backend) Servidor local pára 100ms após responder, ou no timeout.
...
**Notas.** (não executado — efeito colateral: escreve em disco e fala com Rekor) O prompt de publicação é pulado em ambiente não-TTY. `BEHELD_NO_DESKTOP_COPY=1` opta-out da cópia no Desktop.
...
**Notas.** (não executado — efeito colateral: rede + escreve config)
...
**Notas.** (não executado — efeito colateral: rede + abre navegador) Cabeçalho usa `beheld auth` em dim — quebra o padrão visual `▎` de `brand()` do resto do CLI.
...
**Notas.** (não executado — efeito colateral: pode sobrescrever o binário do CLI) Endpoint `/api/version` no backend Rails coberto por `web/source/backend/spec/requests/version_spec.rb`. Flow coberto por `tests/version.test.ts`.
...
| 2 | `beheld delete --local` não tem flag inverso para pular o prompt manual (`apagar tudo`); só o caminho `--all` passa `skipConfirm` internamente. Sem CLI para uso scripted/CI. | `packages/cli/src/commands/delete.ts:278` |
| 3 | `beheld harness install` sem antônimo: existe `list` e `install`, mas não há comando explícito de "uninstall" ou "disable tail". A função `uninstall()` no adapter existe mas não é exposta no CLI. | `packages/cli/src/commands/harness.ts` |
...
| Data | Mudança |
|---|---|
| 2026-06-08 | Snapshot gerado por `/cli-docs` no commit `afa2b6a` com VERSION `0.4.1`. |
